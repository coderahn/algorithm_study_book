...
#### 1)우선순위 큐
  ...
  - 힙을 이용한 우선순위 큐 구현은 Priority라는 우선순위 값을 하나 추가해서 비교값으로 사용
...
#### 2)힙
  ...
  - 삽입, 최소값 삭제 핵심
  - 루트노드는 항상 최소값이다라는 규칙이 기반

### 4.해시테이블

#### 1) 해시테이블?
  - 데이터의 해시 값을 테이블 내의 주소로 이용하는 궁극의 탐색 알고리즘
  - 공간을 팔아서 더 빠르게 접근하는 방법
  - 데이터는 해시함수를 거치면, 테이블 내의 주소값으로 변환되며 이 주소값에 데이터를 저장
  - 특이하게도 해시테이블은 데이터가 입력되지 않은 여유 공간이 많아야 제 성능 유지 가능

#### 2) 해시함수
  - 해시함수 알고리즘은 여러가지가 있음
  - 알고리즘1: 나눗셈법
    - 입력값을 테이블크기로 나눈 나머지가 주소
  - 알고리즘2: 자릿수 접기 
    - 키값이 될 숫자의 각 자리수를 더해서('접다'라고 표현) 새로운 값을 해시값으로 쓰는 것
    - 한자리씩 접을 수도, 두자리씩 접을 수도 있음
    - 8129335는 한자리씩 접을 경우 31
    - 특히 문자열을 키로 사용하는 해시테이블에 잘 어울림, 문자열 각 요소를 아스키 코드 번호로 뽑은 후 더해서 해시값을 만듦
    - 나눗셈법이랑 같이 쓰는듯
  - 해시함수의 충돌?
    - 동일한 주소를 반환한다면 해당 주소(인덱스)에 계속 값이 덮어써질듯..이 충돌은 완전 해결은 안 되지만 어느정도 해결법이 있음
    - 해결방법
      -개방해싱(Open Hashing): 해시테이블 주소 바깥에 새로운 공간 할당
        - 체이닝: 개방해싱 충돌해결기법중 하나로 테이블의 해당 주소에 링크드 리스트 삽입하여 문제 해결
        - 개방주소법
          - 선형 탐사
            - 해시 함수로 얻어낸 주소에 이미 데이터가 있으면 +1 등으로 이동해서 세팅(클러스터 많이 발생)
          - 제곱 탐사
            - 선형 탐사를 보완한 것. 제곱으로 이동폭이 곱으로 늘어나지만 클러스터 가능성 여전
          - 이중 해싱
            - 해시 함수로 얻은 주소로 충돌시 2번째 해시함수로 처리
          - 재해싱
            - 해시테이블 크기를 늘리고 늘어난 해시테이블의 크기에 맞춰 테이블 내의 모든 데이터를 다시 해싱  
      -페쇄해싱(Closed Hashing): 해시테이블 공간 안에서 문제 해결
